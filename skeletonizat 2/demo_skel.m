...
bw=imread('/home/aysylu/Desktop/images/dataset1/root004_8.png');
% bw=imread('/home/aysylu/Desktop/images/binary_batch1/000085C_b.TIF');
...
%     params=no_enclosure(params);
    
    new_bw=zeros(size(latitude{ms},2), size(main_stamm_paths{ms},1));
    O_e = [round(size(latitude{ms},2)/2), round(size(main_stamm_paths{ms},1)/2)];
    
    figure
    imshow(new_bw),hold on
    
    params_2=finish_representation(params,DT);
    plot_ellipse_level(new_bw, params_2, O_e, 'w', 2)
    plot_ellipse_level(new_bw, params, O_e, 'g', 2),hold on
